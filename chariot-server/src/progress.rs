...
impl PlayerEntity {
    ...
    fn get_progress_score(&self, checkpoints: &Vec<Checkpoint>) -> (LapNumber, CheckpointID, f64) {
        // There's a hierarchy of three pieces of progress information we have:
        // from least to most granular lap number, minor checkpoint number, and
        // linear interpolation between minor checkpoint
        ...
        (lap_number, checkpoint_number, position_within_checkpoint)
    }
}
...
pub fn get_player_placement_array(
    ...
    let mut player_nums_with_scores: Vec<(PlayerID, (LapNumber, CheckpointID, f64))> = [0, 1, 2, 3]
        .into_iter()
    ...
    player_nums_with_scores.sort_by(|a, b| {
        let a_arr = [a.1 .0 as f64, a.1 .1 as f64, a.1 .2 as f64];
        let a_finished = players[a.0].lap_info.finished;
        let b_arr = [b.1 .0 as f64, b.1 .1 as f64, b.1 .2 as f64];
