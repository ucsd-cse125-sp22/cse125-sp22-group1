...
fn vs_main([[builtin(vertex_index)]] vertex_index: u32, [[location(0)]] position: vec3<f32>, [[location(1)]] normal: vec3<f32>, [[location(2)]] color: vec3<f32>) -> VertexOutput{
	...
	let world_normal = (mvp.normal_to_global * vec4<f32>(normal, 0.0)).xyz;
	let world_pos_i32 = bitcast<vec3<i32>>(world_pos);
	// TODO: improve, change every frame, etc
	let world_pos_hash = world_pos_i32.x * 73856093 + world_pos_i32.y * 19349669 + world_pos_i32.z * 83492791 + i32(vertex_index);
	...
	let probe_index = world_pos_hash % (probes_grid_size.x * probes_grid_size.y);
	...
	let shade = color * ((shadow * diffuse) + ambient);
