        ...
        *player_transform = Transform::from_entity_location(&location);
        ...
        if player_entity == self.camera_entity {
            if let Some(camera) = self.world.get_mut::<Camera>(self.camera_entity) {
                ...
                let mut orbit_yaw = velocity.z.signum()*velocity_angle - location.unit_steer_direction.z.signum()*rotation_angle;
                ...
                camera.orbit_angle = Vec2::new(orbit_yaw as f32, -0.3).lerp(camera.orbit_angle, 0.5);
            }
        }
    }
