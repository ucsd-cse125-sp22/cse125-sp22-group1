...
impl ResourceManager {
	...
	fn import_gltf(&mut self, renderer : &Renderer, filename : &str) -> core::result::Result<(), gltf::Error>{
		let model_name = filename.split(".").next().expect("invalid filename format");
		let (document, buffers, images) = gltf::import(filename)?;
		let texture_iter = images.iter().map(
			|img| renderer.create_texture(&wgpu::TextureDescriptor{
...
impl Application {
	...
	pub fn render(&mut self) {
		...
		let mut dispatcher = specs::DispatcherBuilder::new()
			.with(StaticMeshRenderSystem::new(&mut self.renderer, &self.resources), "StaticMeshRenderSystem", &[])
			.build();
