...
fn to_wgpu_binding_type(module : &naga::Module, naga_type : &naga::TypeInner) -> wgpu::BindingType {
    match naga_type {
        ...
        naga::TypeInner::Matrix { 
            ...
            ty: wgpu::BufferBindingType::Uniform, 
            has_dynamic_offset: false, 
            min_binding_size: wgpu::BufferSize::new(u64::from(*width) * 
                u64::from(*rows as u8) * u64::from(*columns as u8))
        },
        ...
        naga::TypeInner::Image { 
            ...
            naga::ImageClass::Depth { 
                multi
            } => panic!("depth not supported yet")
        },
        naga::TypeInner::Sampler { 
            comparison 
        } => {
            wgpu::BindingType::Sampler(
                if *comparison { wgpu::SamplerBindingType::Comparison } else { wgpu::SamplerBindingType::Filtering }
            )
        },
...
fn vertex_type_size(ty : &naga::TypeInner) -> u64 {
    match ty {
        ...
        naga::TypeInner::Vector { size, kind, width } => 
            u64::from(*width) * u64::from(*size as u8),
...
fn shader_metadata(source : &str) -> ShaderMetadata {
    ...
    for (_, global_var) in naga_module.global_variables.iter() {
        if let Some(binding) = &global_var.binding {
            ...
            bind_group_layouts.entry(binding.group)
                .or_default()
    ...
    let maybe_vs_fun = naga_module.entry_points.iter()
        .filter(|ep| ep.name.eq(&String::from("vs_main")))
        .last();

    if let Some(vs_fun_pair) = maybe_vs_fun {
        let vs_fun = &vs_fun_pair.function;
        let loc_binding_iter = 
            vs_fun.arguments.iter().filter(|arg| has_location_binding(arg));
...
impl Renderer {
        ...
        match render_pass_desc {
            RenderPassDescriptor::Graphics{
                ...
                let bind_group_layouts = 
                    shader_metadata.bind_group_layouts.iter().map(|(group_num, entries)|
                        self.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor{
                            label: Some(format!("{}_bind_group_layout_{}", name, group_num).as_str()),
                            entries: &entries
                        })
                ).collect::<Vec<wgpu::BindGroupLayout>>();
                self.bind_group_layouts.insert(name.to_string(), bind_group_layouts);

                let vertex_buffer_layouts = 
                    shader_metadata.vertex_attributes.iter().map(|attrib| wgpu::VertexBufferLayout {
                ...
                let pipeline_layout = self.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: None,
                    bind_group_layouts: &self.bind_group_layouts.get(name)
                        .unwrap().iter().collect::<Vec<&wgpu::BindGroupLayout>>(),
    ...
    pub fn create_bind_group(&self, pass_name : &str, group_num : u32, data : &[wgpu::BindingResource]) -> wgpu::BindGroup {
        let bind_group_entries = data.iter().enumerate().map(|(idx, resource)|
            wgpu::BindGroupEntry {
        ...
        self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some(format!("{}_bind_group_{}", pass_name, COUNTER.fetch_add(1, Ordering::Relaxed)).as_str()),
            layout: self.bind_group_layouts.get(pass_name)
