use std::{borrow::Cow, collections::HashMap, sync::atomic::{AtomicUsize, Ordering}};
        ...
        match render_pass_desc {
            RenderPassDescriptor::Graphics{
                ...
                let shader_metadata = shader_metadata(source)
					.expect(format!("Error extracting metadata from shader for pass {}", name).as_str());
    ...
    pub fn render(&mut self, render_job : &RenderJob) {
		...
		// kind of sketch to re-set this every frame
        {
            ...
            self.framebuffers.insert(String::from("surface"), FramebufferDescriptor { 
                ...
                depth_stencil_attachment: Some(self.depth_texture.create_view(&wgpu::TextureViewDescriptor::default())), 
                clear_color: true, 
                clear_depth: true
            });
        }
