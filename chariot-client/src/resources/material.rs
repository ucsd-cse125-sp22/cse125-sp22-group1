...
impl<'a> MaterialBuilder<'a> {
    ...
    pub fn framebuffer_texture_resource<'b>(
        &'b mut self,
        group: u32,
        binding: u32,
        fb_name: &'a str,
        ...
        let fb_tex_view = self
            ...
            .expect("Invalid framebuffer tex")
        ...
        let (res_id, alt_res_id) = if let Some(alt_fb_tex) =
            self.resources.framebuffer_tex(fb_name, idx, true)
        {
        ...
        let alt_bind_groups = self
            ...
            .filter(|(group, resource_map)| {
                resource_map.iter().any(|(binding, resource_idx)| {
                    resource_idx.is_fb_texture_with_alt(self.resources)
                })
            })
